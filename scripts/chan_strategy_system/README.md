...
- **复权方式**：前复权或后复权。
- **V2 多级别分析周期**：默认 `日线,60分钟,30分钟,15分钟`。
...
```powershell
D:\Anaconda\python.exe -u scripts\chan_strategy_system\chan_core.py --mode tushare --symbols 002202.SZ --start-date 20200101 --end-date 20260612 --backtest-start 20200701 --analysis-freqs 日线,60分钟,30分钟,15分钟 --fq 后复权
```
